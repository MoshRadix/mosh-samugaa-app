...
async function migrateFromJSONIfNeeded() {
  ...
  try {
    ...
    for (const t of templates) {
      ...
      `, [
        t.id, t.name, t.description || "", t.category || "General",
        t.filePath, t.originalName, t.type,
        t.hasFields ? 1 : 0, JSON.stringify(t.fields || []),
        t.dateRangeConfig ? JSON.stringify(t.dateRangeConfig) : null,
        t.hasDateRange ? 1 : 0, t.isActive !== false ? 1 : 0,
    ...
    for (const r of dataRecords) {
      db.run(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `, [r.id, r.templateId, JSON.stringify(r.data), r.outputPath || "", r.createdAt]);
    }
    await saveDatabase();
...
async function saveConfig() { await fs.writeFile(CONFIG_PATH, JSON.stringify(settings, null, 2)); }
function getTemplatesDir() { return settings.templatesDir; }
function getOutputsDir() { return settings.outputsDir; }
...
function getTemplateById(id) {
  ...
  return {
    ...
    filePath: row.filePath, originalName: row.originalName, type: row.type,
    hasFields: row.hasFields === 1,
    fields: JSON.parse(row.fields || "[]"),
    dateRangeConfig: row.dateRangeConfig ? JSON.parse(row.dateRangeConfig) : null,
...
function insertTemplate(template) {
  ...
  `, [
    template.id, template.name, template.description || "", template.category || "General",
    template.filePath, template.originalName, template.type,
    template.hasFields ? 1 : 0, JSON.stringify(template.fields || []),
    template.dateRangeConfig ? JSON.stringify(template.dateRangeConfig) : null,
    template.hasDateRange ? 1 : 0, template.isActive !== false ? 1 : 0,
...
function updateTemplateFields(templateId, fields, dateRangeConfig = null) {
  ...
  db.run(`
    ...
    WHERE id = ?
  `, [JSON.stringify(fields || []), hasFields ? 1 : 0,
      dateRangeConfig ? JSON.stringify(dateRangeConfig) : null,
      hasDateRange ? 1 : 0, new Date().toISOString(), templateId]);
}
...
function deleteTemplateRecord(templateId) {
  ...
  if (template && template.filePath) {
    try { fsSync.unlinkSync(template.filePath); } catch(e) {}
  }
...
function getDataRecordCount(templateId) {
  const stmt = db.prepare("SELECT COUNT(*) as count FROM data_records WHERE templateId = ?");
...
function saveDataRecord(record) {
  db.run(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `, [record.id, record.templateId, JSON.stringify(record.data), record.outputPath || "", record.createdAt]);
...
function getDataRecords(templateId) {
  const rows = db.exec("SELECT * FROM data_records WHERE templateId = ? ORDER BY createdAt DESC", [templateId]);
  if (rows.length === 0) return [];
  return rows[0].values.map(row => ({
    id: row[0], templateId: row[1], data: JSON.parse(row[2]), outputPath: row[3], createdAt: row[4]
  }));
}
...
function processDateRangePlaceholders(fields) {
  ...
  const startDateField = {
    ...
    value: "", isRTL: false, originalKey: "date_range_start"
  };
  otherFields.push(startDateField);
  const dateRangeConfig = {
    enabled: true, startKey: "date_range_start", count: maxIndex,
  ...
  return withAutoSave(async () => {
    ...
    if (extension === ".docx") {
      try {
        ...
        fields = matches.filter(m => { const k = m[1].trim(); if (uniqueKeys.has(k)) return false; uniqueKeys.add(k); return true; }).map(m => {
          const key = m[1].trim();
          const isDivehi = /divehi/i.test(key);
          const isDate = /date/i.test(key);
          const cleanKey = key.replace(/^(divehi[._]|date[._])/, "");
          return { key, label: cleanKey.replace(/[._]/g, " ").replace(/\b\w/g, l => l.toUpperCase()), type: isDate ? "date" : "string", required: false, value: "", isRTL: isDivehi, originalKey: key };
        });
              ...
              if (cell.value && typeof cell.value === "string") {
                ...
                matches.forEach(m => {
                  ...
                  if (!uniqueKeys.has(key)) {
                    ...
                    const isDate = /date/i.test(key);
                    const cleanKey = key.replace(/^(divehi[._]|date[._])/, "");
                    placeholders.push({ key, label: cleanKey.replace(/[._]/g, " ").replace(/\b\w/g, l => l.toUpperCase()), type: isDate ? "date" : "string", required: false, value: "", isRTL: isDivehi, originalKey: key });
                  }
                });
              }
            });
    ...
    const template = {
      id: templateId, name: metadata.name || fileName.replace(extension, ""),
      description: metadata.description || "", category: metadata.category || "General",
      filePath: templatePath, originalName: fileName, type: type,
      hasFields: fields.length > 0, fields: fields, dateRangeConfig: dateRangeConfig,
      hasDateRange: !!dateRangeConfig, isActive: true,
...
ipcMain.handle("reload-template-fields", async (event, templateId) => {
  return withAutoSave(async () => {
    ...
    if (extension === ".docx") {
      ...
      const matches = [...fullText.matchAll(regex)];
      const uniqueKeys = new Set();
      newFields = matches.filter(m => { const k = m[1].trim(); if (uniqueKeys.has(k)) return false; uniqueKeys.add(k); return true; }).map(m => {
        const key = m[1].trim();
        const isDivehi = /divehi/i.test(key);
        const isDate = /date/i.test(key);
        const cleanKey = key.replace(/^(divehi[._]|date[._])/, "");
            ...
            if (cell.value && typeof cell.value === "string") {
              ...
              matches.forEach(m => {
                ...
                if (!uniqueKeys.has(key)) {
                  ...
                  const isDate = /date/i.test(key);
                  const cleanKey = key.replace(/^(divehi[._]|date[._])/, "");
                  newFields.push({ key, label: cleanKey.replace(/[._]/g, " ").replace(/\b\w/g, l => l.toUpperCase()), type: isDate ? "date" : "string", required: false, value: "", isRTL: isDivehi, originalKey: key });
                }
              });
            }
          });
...
ipcMain.handle("generate-document", async (event, { templateId, formData, outputFormat = "docx" }) => {
  ...
  if (template.type === "word") await generateWordDocument(template.filePath, outputPath, formData);
  else if (template.type === "excel") await generateExcelDocument(template.filePath, outputPath, formData);
  else throw new Error("Unsupported template type");
  const record = { id: uuidv4(), templateId: template.id, data: formData, outputPath, createdAt: new Date().toISOString() };
...
ipcMain.handle("save-data-record", async (event, { templateId, data }) => {
  const record = { id: uuidv4(), templateId, data, createdAt: new Date().toISOString(), outputPath: "" };
  saveDataRecord(record);
  await saveDatabase();
  return record;
});
ipcMain.handle("get-data-records", async (event, templateId) => getDataRecords(templateId));
...
ipcMain.handle("export-document", async (event, { sourcePath, suggestedName }) => {
  const result = await dialog.showSaveDialog(mainWindow, {
    title: "Export Document", defaultPath: suggestedName,
    filters: [{ name: "Word Document", extensions: ["docx"] }, { name: "Excel Spreadsheet", extensions: ["xlsx"] }, { name: "All Files", extensions: ["*"] }]
  });
...
ipcMain.handle("get-app-info", () => ({
  version: app.getVersion(), name: app.getName(), electronVersion: process.versions.electron,
...
ipcMain.handle("open-file-dialog", async () => {
  const result = await dialog.showOpenDialog(mainWindow, {
    properties: ["openFile"],
    filters: [{ name: "Document Templates", extensions: ["docx", "xlsx", "pdf"] }]
  });
...
ipcMain.handle("get-settings", () => ({
  dataDir: settings.dataDir, templatesDir: settings.templatesDir,
  outputsDir: settings.outputsDir, dbPath: settings.dbPath
}));
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    title: "Document Generator", autoHideMenuBar: true, show: false
  });
